...
3.  **按需生成 μ可畏鼠标指针:**
    ...
    需要 Python 3 与 ImageMagick。将个人持有的原始 ZIP 传给转换脚本，生成的主题可在 KDE 或 GTK 的光标设置中选择为 `μ可畏 Pointer`：

    ```bash
    python3 .icons/build-mukewei-pointer.py \
        ~/Downloads/μ可畏-鼠标指针.zip \
        ~/.icons/mukewei-pointer
    ```
